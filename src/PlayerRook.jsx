...
export default function PlayerRook({myWantedPos, myWantedPosChanged, position}){
    ...
    const handleInput = (event)=>{      
        ...
        if(event.key === "ArrowUp" && x.current <=1 && z.current == 0){
        ...
        else if(event.key === "ArrowDown" && x.current >=-1 && z.current == 0){
         
          x.current -= 1;
          myWantedPos.current = {x:-1, z:0};
          myWantedPosChanged();
        }
        else if(event.key === "ArrowLeft" && z.current >=-1 && x.current == 0){
          
          z.current -= 1;
          myWantedPos.current = {x:0, z:-1};
          myWantedPosChanged();
        }
        else if(event.key === "ArrowRight" && z.current <=1 && x.current == 0){
